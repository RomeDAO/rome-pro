...
contract MockRomeTreasury is Ownable {
  using SafeMath for uint256;
  ...
  uint256 public sROMEQueue; // Delays change to sROME address
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sROME).balanceOf(msg.sender); // Can only borrow against sROME held
    ...
    returns (uint256 value_)
  {
    if (isReserveToken[_token]) {
      ...
      value_ = _amount.mul(10**IERC20(ROME).decimals()).div(
