...
contract GenericBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair, address _mainToken ) external view returns ( uint ) {
        ( uint reserve0, uint reserve1, ) = IUniswapV2Pair( _pair ).getReserves();
        ...
        return reserve.mul( 2 * ( 10 ** IERC20( _mainToken ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}
